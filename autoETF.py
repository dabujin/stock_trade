...
myToken = "xoxb-2049051962662-2053907188695-AcmihT2jPeXoI1ORH5IsqpUS"
def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    post_message(myToken,"#creonetf", strbuf)
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.5  #hj, 변동성이 얼마인지 확인(초기0.5)
        return target_price
...
if __name__ == '__main__': 
    try:
        ...
        while True:
            t_now = datetime.now()
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0)      #프로그램 시작시간
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0)  #9시 5분에 매매시작
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) #매도시간
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0)  #프로그램 종료시간
            today = datetime.today().weekday()
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_9 < t_now < t_start and soldout == False:  #남아있는 종목 모두 매도
                soldout = True
                sell_all()
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count:
                        buy_etf(sym)
                        time.sleep(1)
                if t_now.minute == 30 and 0 <= t_now.second <= 5:  #30분마다 잔고 전송
